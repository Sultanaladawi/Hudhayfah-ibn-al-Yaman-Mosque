  ...
  return (
    <div style={{ 
      ...
      <style>{`
        ...
        .input-group:focus-within .label-text { color: ${colors.latte}; letter-spacing: 1px; }
      `}</style>
      ...
      <div className="login-card">
        <div style={{ textAlign: 'center', marginBottom: '40px' }}>
          ...
          <h1 style={{ fontFamily: "'Amiri', serif", fontSize: '2.2rem', color: '#fff', margin: '0 0 8px 0', lineHeight: 1 }}>
            مسجد <span style={{ color: colors.latte, fontStyle: 'italic' }}>حذيفة بن اليمان</span>
          </h1>
          <p style={{ color: 'rgba(255,255,255,0.25)', fontSize: '0.85rem', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '2px' }}>
            لوحة الإدارة الآمنة
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '22px' }}>
          <div className="input-group">
            <label className="label-text"><Mail size={12} /> البريد الإلكتروني</label>
            <input type="email" className="lux-input" placeholder="admin@huzaifa-mosque.com" value={form.email} onChange={e => setForm({...form, email: e.target.value})} required autoFocus style={{textAlign: 'left'}} />
          </div>

          <div className="input-group">
            <label className="label-text"><Lock size={12} /> كلمة المرور</label>
          ...
          <button type="submit" disabled={loading} className="login-btn">
            {loading ? <Loader2 className="animate-spin" size={20} /> : <>تسجيل الدخول <Zap size={16} fill="#070504" /></>}
          </button>
        </form>

        <div style={{ marginTop: '40px', textAlign: 'center' }}>
          <div style={{ height: '1px', background: `linear-gradient(90deg, transparent, ${colors.border}, transparent)`, marginBottom: '15px' }} />
          <p style={{ color: 'rgba(255,255,255,0.2)', fontSize: '0.65rem', fontWeight: 900, letterSpacing: '0px' }}>
            نظام إدارة المساجد
          </p>
        </div>
      </div>
